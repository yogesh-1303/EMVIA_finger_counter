...
int main(int argc, char *argv[]){
	...
	for(int j=1; j<=NUM_IMGS; j++){
		String filename = format("OUT_hand%d.pgm", j);
		String filename2 = format("IN_hand%d.ppm", j);
		String filename3 = format("DETECT%d.ppm", j);
		Mat skel = imread(filename, IMREAD_UNCHANGED);
		Mat frame = imread(filename2, IMREAD_UNCHANGED);
		HoughLinesP(skel, lines, 1, CV_PI/180,15, 20, 10 );
